...
function AnalogClock({ timeLeft, totalDuration, danger, critical }) {
    ...
    const baseColor = critical ? 'rgba(239,68,68,' : danger ? 'rgba(249,115,22,' : 'rgba(59,130,246,';
    const accentColor = critical ? '#ef4444' : danger ? '#f97316' : '#3B82F6';
...
function DigitBlock({ value, label, danger, critical }) {
    ...
    const textColor = critical ? 'text-red-500' : danger ? 'text-orange-500' : 'text-[#00FFFF]';
    const shadowColor = critical ? 'rgba(239,68,68,0.8)' : danger ? 'rgba(249,115,22,0.8)' : 'rgba(0,255,255,0.8)';
    const borderColor = critical ? 'border-red-500/50' : danger ? 'border-orange-500/50' : 'border-[#3B82F6]/30';
    const bgColor = critical ? 'bg-red-500/10' : danger ? 'bg-orange-500/10' : 'bg-black/60';

    return (
        <div className="flex flex-col items-center gap-2">
            <div className={`relative w-[25vw] h-[25vw] max-w-[110px] max-h-[110px] sm:max-w-[150px] sm:max-h-[150px] sm:w-[150px] sm:h-[150px] ${bgColor} ${borderColor} border-2 rounded-2xl flex items-center justify-center shadow-[0_0_30px_rgba(0,255,255,0.1),inset_0_2px_0_rgba(0,255,255,0.2)] glass-panel overflow-hidden`}>
                {/* Scan line */}
            ...
            <span className={`text-[10px] sm:text-xs uppercase tracking-[0.3em] font-bold ${critical ? 'text-red-400' : danger ? 'text-orange-400' : 'text-[#00FFFF]/70'}`}>{label}</span>
        </div>
    );
}
...
export default function WaitingRoom({ onTimerComplete, onLogout, initialStartTime }) {
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-black text-slate-300 font-mono-custom">
                <div className="w-12 h-12 border-2 border-[#00FFFF] border-t-transparent rounded-full animate-spin mb-4" />
                <p className="text-xs tracking-wider text-[#00FFFF] font-bold">SYNCING WITH PORTAL SERVER...</p>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen py-10 px-4 relative flex items-center justify-center font-mono-custom bg-[var(--bg-matte)]">
            ...
            <div className={`w-full max-w-4xl mx-auto glass-panel border-4 ${isCritical ? 'border-red-500' : isDanger ? 'border-orange-500' : 'border-[#00FFFF]'} rounded-none shadow-[0_15px_50px_rgba(0,0,0,0.8)] relative overflow-hidden z-20 transition-colors duration-500`}>
                ...
                <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-[#00FFFF]/50 to-transparent" />
                ...
                <div className="flex items-center justify-between px-6 pt-8 pb-4 border-b border-zinc-800/80">
                    <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-xs text-[#00FFFF] font-bold uppercase tracking-wider">
                            Connected: {teamId}
                        </span>
                    </div>
                    <button
                ...
                <div className="px-6 py-8 flex flex-col items-center gap-8 select-none">
                    ...
                    <div className="text-center">
                        <p className="text-[10px] uppercase tracking-[0.25em] text-[#00FFFF] font-semibold mb-1 font-orbitron">
                            DISFRUTAR-2K26 LAUNCH COUNTDOWN
                        </p>
                    ...
                    <div className="flex flex-col items-center gap-8 my-8 relative w-full border border-zinc-800/80 bg-black/40 p-8">
                        ...
                        <div className="absolute -top-3 bg-black px-2 text-[10px] uppercase tracking-[0.3em] text-[#00FFFF] font-bold">System Calibration</div>
                        ...
                        <div className="flex items-center gap-2 sm:gap-6">
                            ...
                            <div className="flex flex-col items-center justify-center h-[110px] sm:h-[150px] pb-[25px]">
                                <span className={`text-5xl sm:text-7xl font-bold leading-none animate-pulse ${isCritical ? 'text-red-500' : isDanger ? 'text-orange-500' : 'text-[#00FFFF]/60'}`} style={{
                                    textShadow: `0 0 20px ${isCritical ? 'rgba(239,68,68,0.5)' : isDanger ? 'rgba(249,115,22,0.5)' : 'rgba(0,255,255,0.5)'}`
                                }}>:</span>
                            </div>
                    ...
                    {/* Anti-cheat banner */}
                    <div className="w-full bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 flex gap-3 text-left font-sans">
                        <AlertTriangle className="w-5 h-5 text-[#00FFFF] shrink-0 mt-0.5" />
                        <div>
                            <h4 className="text-[#00FFFF]/90 text-xs font-bold uppercase tracking-wider mb-1">Strict Anti-Cheat Policy Active</h4>
                        ...
                        <div className="space-y-1.5 text-[10px] text-slate-400 max-h-28 overflow-y-auto scrollbar-none">
                            {systemLogs.map((log, idx) => (
                                <div key={idx} className="flex gap-2">
                                    <span className="text-[#00FFFF]/70">&gt;</span>
                                    <span>{log}</span>
                                </div>
                            ))}
                        </div>
                ...
                <div className="absolute bottom-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-[#00FFFF]/30 to-transparent" />
            </div>
        </div>
    );
}
