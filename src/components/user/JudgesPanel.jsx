...
export default function JudgesPanel({ onBack }) {
  ...
  const chiefGuests = [
    ...
    {
      id: "judge-2",
      name: "Premsai Varma Chekuri",
      initials: "PV",
      Role: "AI Engineer",
      company: "TCS",
      badgeColor: "gold",
      description: "AI Engineer designing and deploying agentic AI pipelines that solve real enterprise challenges with LangChain, LangGraph, Azure OpenAI, Python, and Vector Databases. Active open-source contributor and technical mentor passionate about bridging complex AI systems and explainable applications."
    }
  ];
      ...
      >
        ...
        >
          <div className="flex items-center gap-4">
            <button
              onClick={onBack}
              className="group flex items-center justify-center p-3 rounded-2xl border border-white/10 hover:border-[#00FF66]/50 bg-black/60 text-slate-300 hover:text-[#00FF66] transition-all duration-300 cursor-pointer shadow-lg hover:shadow-[0_0_15px_rgba(0,255,102,0.2)]"
            >
            ...
            <div>
              <h1 className="flex items-center font-orbitron font-extrabold text-white leading-none select-none text-xl md:text-2xl tracking-wider">
                <span className="text-[#00FF66] mr-2">DSFRUTAR-2K26</span>
        ...
        <motion.div variants={itemVariants} className="mb-6">
          ...
          >
            <span className="bg-amber-400/15 text-amber-400 px-2.5 py-0.5 rounded-full border border-amber-400/40 text-[10px] inline-flex font-mono-custom font-bold">VIP</span>
            ...
            {chiefGuests.map((guest) => {
              ...
              return (
                <div
                  ...
                  className="relative bg-black/50 border border-white/10 hover:border-yellow-400/60 rounded-2xl p-8 flex flex-col items-center justify-center text-center gap-5 transition-all duration-300 group shadow-lg hover:shadow-[0_0_30px_rgba(255,215,0,0.15)] hover:scale-[1.01] cursor-pointer overflow-hidden backdrop-blur-xl"
                >
                  {/* Glowing corner accent */}
                  <div className="absolute top-0 right-0 w-24 h-24 bg-yellow-400/5 rounded-full blur-2xl group-hover:bg-yellow-400/15 transition-colors" />

                  {/* Pure Vector VIP Holographic Avatar Badge */}
                  <div className="relative shrink-0">
                    <div className="w-24 h-24 md:w-28 md:h-28 rounded-2xl bg-gradient-to-tr from-black via-zinc-900 to-zinc-800 border-2 border-yellow-400/50 group-hover:border-yellow-400 flex items-center justify-center shadow-[0_0_20px_rgba(255,215,0,0.25)] relative z-10 transition-transform duration-300 group-hover:scale-105">
                      <span className="text-2xl md:text-3xl font-black font-orbitron text-yellow-400 drop-shadow-[0_0_10px_rgba(255,215,0,0.5)]">
                        {guest.initials}
                      </span>
                      {/* Orbital Corner Ring */}
                      <div className="absolute -bottom-2 -right-2 p-1.5 rounded-lg bg-black border border-yellow-400/60 shadow">
                  ...
                  <div className="min-w-0 flex flex-col items-center w-full">
                    <h4 className="text-base md:text-lg font-bold text-white uppercase font-orbitron truncate w-full group-hover:text-yellow-400 transition-colors">
                      {guest.name}
                    </h4>
                    <p className="text-xs md:text-sm text-emerald-400 font-mono-custom font-semibold mt-1 truncate w-full">
                      {guest.Role}
                    </p>
                  ...
                  <span className="text-[10px] text-yellow-400/80 font-mono-custom uppercase tracking-wider group-hover:underline">
        ...
        {selectedJudge && (
          ...
          >
            <motion.div
              ...
              className="bg-[#0c0f17] border border-yellow-400/40 rounded-3xl max-w-lg w-full relative shadow-[0_0_50px_rgba(255,215,0,0.2)] flex flex-col overflow-hidden max-h-[90dvh]"
            >
              <button
                onClick={() => setSelectedJudge(null)}
              ...
              <div className="p-8 overflow-y-auto">
                <div className="flex flex-col items-center text-center mt-2">
                  ...
                  <div className="w-24 h-24 rounded-2xl bg-gradient-to-tr from-black via-zinc-900 to-zinc-800 border-2 border-yellow-400 flex items-center justify-center shadow-[0_0_30px_rgba(255,215,0,0.3)] mb-4">
                    <span className="text-3xl font-black font-orbitron text-yellow-400">
                      {selectedJudge.initials}
                    </span>
                  </div>

                  <h3 className="text-xl font-bold text-white font-orbitron mb-1">{selectedJudge.name}</h3>
                  <p className="text-emerald-400 font-mono-custom font-semibold text-sm mb-1">{selectedJudge.Role}</p>
